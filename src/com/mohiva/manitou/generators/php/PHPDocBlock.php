...
class PHPDocBlock extends Generator {
	...
	public function generate() {
		...
		$code .= $this->sections && $this->annotations ? ' * ' . self::LINE_FEED : '';
	...
	public function generateSections() {
		...
		foreach ($this->sections as $section) {
			...
			$first = false;
			$lines = explode(self::LINE_FEED, trim($section, self::LINE_FEED));
			
			// Wrap long lines without a newline character
			if (count($lines) == 1) {
				$code .= ' * ' . wordwrap($section, self::SECTION_WIDTH, self::LINE_FEED . ' * ') . self::LINE_FEED;
				continue;
			}
			
			// Build lines
			foreach ($lines as $line) {
				$code .= ' * ' . $line . self::LINE_FEED;
			}
		}
	...
	public function generateAnnotations() {
		...
		foreach ($this->annotations as $annotation) {
			$code .= ' * ' . $annotation . self::LINE_FEED;
		}
